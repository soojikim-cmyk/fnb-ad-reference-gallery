...
.tabrow .seg button:disabled{opacity:.32;cursor:not-allowed}  /* 현재 대분류에 0건인 탭 */
...
// 현재 대분류(cat) 기준 탭별 건수
function catCount(t){let n=0;for(const a of ADS){if(a.src===t&&(!cat||a.category===cat))n++;}return n;}
// 탭 뱃지 카운트를 현재 대분류에 맞춰 갱신 + 0건 탭은 비활성(클릭 불가)
function updateTabs(){
  [...$('#tabs').children].forEach(b=>{
    const n=catCount(b.dataset.tab);
    const tc=b.querySelector('.tc'); if(tc)tc.textContent=n;
    b.disabled=n===0;
  });
}
function setCat(c){
  ...
  updateTabs();
  // 현재 탭이 이 대분류에서 0건이면 → 항목 있는 첫 탭으로 자동 이동(대분류 유지) → 빈 화면 방지
  if(c && catCount(tab)===0){
    const nb=[...$('#tabs').children].find(b=>catCount(b.dataset.tab)>0);
    if(nb){setTab(nb.dataset.tab,true);return;}
  }
  rebuildOptions();
  apply();
}
...
function setTab(t,keepCat){
  ...
  if(!keepCat){  // 탭을 직접 누르면 대분류 전체로 리셋, 대분류 선택에 의한 자동이동이면 유지
    cat='';
  ...
  updateTabs();      // 탭 카운트를 현재 대분류에 맞춰 갱신
  rebuildOptions();  // 탭+대분류에 맞춰 브랜드·포맷·후킹·소구·톤 옵션 재구성
  syncHashtags();    // 해시태그 드롭다운을 그룹별 optgroup 으로 재구성
  closePeek();
  apply();
}
$('#tabs').addEventListener('click',e=>{const b=e.target.closest('button[data-tab]');if(b&&!b.disabled)setTab(b.dataset.tab)});
setTab('meta_ad');
</script>
</body></html>`;
